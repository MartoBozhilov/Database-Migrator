-- School System Database for MS SQL Server
-- Test database for ETL system Phase 3

-- Drop and create database
-- Note: Run this with appropriate permissions (sa or database owner)
-- Execute: sqlcmd -S localhost -U sa -P YourPassword -i school_system_mssql.sql

USE master;
GO

IF EXISTS (SELECT name FROM sys.databases WHERE name = 'school_system')
BEGIN
...
-- Note: MS SQL Server doesn't support ON DELETE CASCADE for multiple cascade paths
-- The enrollments -> courses FK doesn't have CASCADE because courses -> teachers has SET NULL
-- If you delete a course, you need to manually delete enrollments first
